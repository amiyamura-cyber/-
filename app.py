...
if search_trigger:
    if not keyword:
        ...
        huge_fallback_pool = [
            ...
            "ターゲット", "ペルソナ", "ブランディング", "カルチャー", "風土", "環境", "制度", "福利厚生", "待遇", "給与",
            "リモート", "テレワーク", "柔軟", "多様性", "女性", "シニア", "外国人", "グローバル",
            "デジタル", "DX", "IT", "スキルアップ", "リスキリング", "モチベーション", "定着率", "離職率",
            "効率化", "自動化", "ツール", "システム", "応募", "受付", "対応", "スピード", "オンボーディング",
            "ノウハウ", "マニュアル", "一元管理", "辞退防止", "内定者", "ウェルカム", "定着支援", "面談"
        ]
...
if not st.session_state.search_clicked:
    ...
    if selected_to_register:
        st.write("")
        if st.button("📥 選択したキーワードを一覧に記憶する（次回から完全除外）", type="primary", use_container_width=True):
    ...
    # ★★★【修正の核心】グループごとに改行し、グループ内は横一列・半角スペース区切りで出力 ★★★
    lines = []
    for i, group in enumerate(st.session_state.cur_grouped_keywords):
        lines.append(f"【グループ{i+1}】 " + " ".join(group))
    keywords_text = "\n".join(lines)
    
    st.write("")
    st.text_area(label="📋 コピペ用テキストエリア（右上のボタンで一括コピーできます）", value=keywords_text, height=150)
    
    st.write("---")
    
    st.markdown("### 📰 関連する注目記事（重複・記憶ワード完全排除 最大10選）")
